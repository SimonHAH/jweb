...
<div id="templatemo_wrapper">
        ...
        <div id="content" class="float_r">
        	<div id="slider-wrapper">
                <div id="slider" class="nivoSlider">
                	<c:forEach items="${productTop}" var="product">
                		<img src="${pageContext.request.contextPath}${product.s_imgurl}" style="width: 680px;height: 300px" alt="" title="${product.s_pname }"/>
                	</c:forEach>
                </div>
        	...
        	<c:forEach items="${hotProducts }" var="product" varStatus="vs">
        		<div class="${vs.index % 3 != 2?'product_box':'product_box no_margin_right' }">
		            <h3>${product.s_pname }</h3>
	            	<a href="${pageContext.request.contextPath }/ProductServlet?op=findProductById&pid=${product.s_pid}">
	            		<img src="${product.s_imgurl }" width="200" height="150" alt="" title="${product.s_pname }"/>
	            	</a>
	                <p style="width: 200px;overflow: hidden;white-space: nowrap; text-overflow: ellipsis;">${product.s_desc }</p>
	                <p class="product_price">￥ ${product.s_estoreprice }</p>
	                <c:if test="${empty user }">
	                	<a href="javascript:login()" class="addtocart"></a>
	                </c:if>
	                <c:if test="${!empty user }">
	                	<a href="javascript:addCart(${product.s_pid },${user.s_uid})" class="addtocart"></a>
	                </c:if>
	                <a href="${pageContext.request.contextPath }/ProductServlet?op=findProductById&pid=${product.s_pid}" class="detail"></a>
            	</div>
	...
	<script type="text/javascript">
		...
		function addCart(pid,uid){
			window.location.href="${pageContext.request.contextPath}/CartServlet?op=addCart&pid="+pid+"&uid="+uid+"&s_num=1";
		}
	</script>
</body>
</html>