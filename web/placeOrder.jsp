	...
	<div style="padding-top: 30px;">
		<table style="width: 700px;border-style:1px solid " border="1">
			...
			<c:forEach items="${shoppingCar.shoppingItems}" var="item">
				<input type="hidden" name="num" value="${item.s_num}"/>
				<tr id="tr${item.product.s_pid}">
					<td><input type="checkbox" class="ids" name="ids" value="${item.product.s_pid}" checked></td>
					<td><img src="${item.product.s_imgurl}" width="100" height="100" /></td>
					<td>${item.product.s_pname}</td>
					<td>${item.s_num}</td>
					<td>${item.product.s_estoreprice}</td>
					<td id="price">${item.s_num*item.product.s_estoreprice}</td>
				</tr>
				<c:set var="sum" value="${sum+item.s_num*item.product.s_estoreprice}"> </c:set>
			</c:forEach>
		</table>
	</div>
