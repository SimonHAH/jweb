...
<div id="templatemo_wrapper">
	...
	<table border="1" width="700" style="border-style: inherit; border-color: black;">
		...
		<c:forEach items="${orders }" var="order"  varStatus ="status">
			<tr style="color: grey">
				<td>${status.count}</td>
				<td>${order.s_ordertime}</td>
				<td>${order.s_money }</td>
				<td>
					<c:if test="${order.s_state == 0}">订单已取消</c:if>
					<c:if test="${order.s_state == 1}">已下单</c:if>
					<c:if test="${order.s_state == 2}">已支付</c:if>
					<c:if test="${order.s_state == 3}">已发货</c:if>
				</td>
				<td>
					<c:if test="${order.s_state==1}"><a href="${pageContext.request.contextPath}/OrderServlet?op=cancelOrder&oid=${order.s_oid}&state=0">取消订单</a>
