		...
		<div id="templatemo_main">
			...
			<div id="content" class="float_r">
					...
					<table width="680px" cellspacing="0" cellpadding="5">
						...
						<c:if test="${!empty user }">
							<c:forEach items="${shoppingCar.shoppingItems}" var="item">
								<tr>
									<td><img src="${item.product.s_imgurl }" style="width: 200px;height: 180px" alt="" /></td>
									<td>${item.product.s_pname }</td>
									<td align="center"><font style="width: 20px; text-align: right" />${item.s_num }</font> </td>
									<td align="right">${item.product.s_estoreprice } </td>
									<td align="right">${item.product.s_estoreprice*item.s_num } </td>
									<td align="center">
										<a href="${pageContext.request.contextPath }/CartServlet?op=delItem&uid=${user.s_uid}&itemid=${item.s_itemid}">
				...
				<div style="float:right; width: 255px; margin-top: 20px;">
					...
					<c:if test="${!empty shoppingCar.shoppingItems}"><p><a href="${pageContext.request.contextPath }/placeOrder.jsp">立即购买</a></p></c:if>
